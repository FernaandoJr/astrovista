...
import { PrismaClient, type Pictures } from '../../prisma/generated/client.js'
import { errorResponse } from '../utils/errorResponse.js'
import { handlerLinks } from '../utils/handlerLinks.js'
import { searchResponse } from '../utils/searchResponse.js'
import { validateSearchParams } from '../utils/validateSearchParams.js'
...
apods.get('/cron', async (c) => {
  // Verificar se é uma chamada válida do Vercel Cron
  const authHeader = c.req.header('authorization')
  if (authHeader !== `Bearer ${process.env.CRON_SECRET}`) {
    return c.json(errorResponse('Unauthorized', 'Invalid cron authorization', 401), 401)
  }
  ...
  try {
    ...
    if (exists) {
      ...
      return c.json({ message: 'APOD already exists for this date' }, 200)
    }
    ...
    console.log('✅ [CRON] Novo APOD salvo com sucesso:', newApod.title)
    return c.json({ message: 'APOD saved successfully', title: newApod.title }, 201)
  } catch (error: any) {
    console.error('❌ [CRON] Erro ao buscar novo APOD:', error.message)
    return c.json(errorResponse('Cron job failed', error.message, 500), 500)
  }
})
